...
function list_weathermaps($mapdir)
{
    ...
    //
    global $WEATHERMAP_VERSION, $configerror, $action;
...
{
	global $mapdir, $WEATHERMAP_VERSION, $configerror;
...
function tidy_link(&$map,$target, $linknumber=1, $linktotal=1, $ignore_tidied=FALSE)
{
	...
	if(isset($map->links[$target]) and isset($map->links[$target]->a) ) {
		...
		if ( !$y_overlap && !$x_overlap ) {
			...
			$pt_a = new Point($node_a->x, $node_a->y);
			$pt_b = new Point($node_b->x, $node_b->y);


			$line = new LineSegment($pt_a, $pt_b);
